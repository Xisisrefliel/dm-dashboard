...
function DiceRoller(): React.ReactElement {
  const [result, setResult] = useState<DiceEntry | null>(null);
  const [rolling, setRolling] = useState<boolean>(false);
  const [history, setHistory] = useState<DiceEntry[]>([]);
  const dice: number[] = [4, 6, 8, 10, 12, 20, 100];

  const roll = (sides: number): void => {
    setRolling(true);
    setTimeout(() => {
      const val: number = Math.floor(Math.random() * sides) + 1;
      const entry: DiceEntry = { sides, val, id: Date.now() };
      setResult(entry);
  ...
  const isCrit: boolean = !!(result && result.sides === 20 && result.val === 20);
  const isFumble: boolean = !!(result && result.sides === 20 && result.val === 1);
  const isMax: boolean = !!(result && result.val === result.sides && result.sides !== 20);
