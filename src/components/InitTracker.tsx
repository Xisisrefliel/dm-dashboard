...
interface InitTrackerProps {
  campaignId: string;
  prefill: InitPrefill | null;
  onPrefillConsumed?: () => void;
}

interface SavedInitState {
  entries: InitEntry[];
  active: number;
}

function loadState(campaignId: string): SavedInitState | null {
  try {
...
function saveState(campaignId: string, entries: InitEntry[], active: number): void {
  localStorage.setItem(`dm-init-${campaignId}`, JSON.stringify({ entries, active }));
}

function InitTracker({ campaignId, prefill, onPrefillConsumed }: InitTrackerProps) {
  const [entries, setEntries] = useState<InitEntry[]>(() => loadState(campaignId)?.entries || []);
  const [name, setName] = useState<string>("");
  const [init, setInit] = useState<string>("");
  const [hp, setHp] = useState<string>("");
  const [active, setActive] = useState<number>(() => loadState(campaignId)?.active ?? -1);
  ...
  useEffect(() => {
    const handler = (e: CustomEvent<{ name: string; hp: number | null }>) => {
    ...
    window.addEventListener("dm-add-initiative", handler as EventListener);
    return () => window.removeEventListener("dm-add-initiative", handler as EventListener);
  }, []);
