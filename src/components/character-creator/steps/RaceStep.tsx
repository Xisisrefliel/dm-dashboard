...
import { RACE_ICONS, ABILITY_NAMES } from "../../../data/character-constants.ts";
import RaceCard from "../cards/RaceCard.tsx";
import TraitChip from "../cards/TraitChip.tsx";
import { useCarouselOverflow } from "../../../hooks/useCarouselOverflow.ts";
import type { Character, SrdRace } from "../../../types/index.ts";
...
interface Props {
  ...
  previewRace: string | null;
  setPreviewRace: (id: string | null) => void;
  raceCarouselRef: RefObject<HTMLDivElement | null>;
  raceCarouselOverflows: boolean;
}

export default function RaceStep({ char, update, next, prev, haptic, previewRace, setPreviewRace, raceCarouselRef, raceCarouselOverflows }: Props) {
  const isMobile = useIsMobile();
  const [closing, setClosing] = useState<boolean>(false);

  const pRace = previewRace ? (racesData as SrdRace[]).find((r) => r.id === previewRace) : null;
  const pRaceImg = previewRace ? (RACE_IMAGES as Record<string, string>)[previewRace] : null;

  const openRacePreview = (id: string): void => {
        ...
        <div key={previewRace!} style={{
          ...
          {pRaceImg && (
            <div style={isMobile
              ...
              <img src={pRaceImg} alt={pRace.name} style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "top", display: "block", outline: "1px solid rgba(255,255,255,0.1)", outlineOffset: -1 }} />
            </div>
          )}
          ...
          <div style={isMobile
            ...
            <h3 style={{ fontSize: 24, fontWeight: 700, margin: "0 0 8px", letterSpacing: "0.02em", textTransform: "uppercase", textWrap: "balance" }}>
              {pRace.name}
            </h3>
            <p style={{ fontSize: 13, color: "var(--dm-text-secondary)", lineHeight: 1.5, margin: "0 0 12px", textWrap: "pretty" }}>
              {pRace.age}
            </p>
