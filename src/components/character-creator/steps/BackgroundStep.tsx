...
import { BG_IMAGES } from "../../../data/character-images.ts";
import { BACKGROUNDS, SKILL_DESCRIPTIONS } from "../../../data/character-constants.ts";
...
interface Props {
  ...
  setPreviewBg: (id: string | null) => void;
  bgCarouselRef: RefObject<HTMLDivElement | null>;
  bgCarouselOverflows: boolean;
}

export default function BackgroundStep({ char, update, next, prev, haptic, previewBg, setPreviewBg, bgCarouselRef, bgCarouselOverflows }: Props) {
  const isMobile = useIsMobile();
  const [closing, setClosing] = useState<boolean>(false);

  const pBg = previewBg ? BACKGROUNDS.find((b) => b.id === previewBg) : null;
  const pBgImg = previewBg ? (BG_IMAGES as Record<string, string>)[previewBg] : null;

  const openBgPreview = (id: string): void => {
    ...
    return (
      <div>
        ...
        <div style={{ position: "relative" }}>
          ...
          >
            {BACKGROUNDS.map((bg) => {
              const img = (BG_IMAGES as Record<string, string>)[bg.id];
