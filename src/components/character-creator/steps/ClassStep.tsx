...
import { CLASS_ICONS, CLASS_DESCRIPTIONS, CLASS_PRIMARY_ABILITY, SKILL_DESCRIPTIONS, ABILITY_NAMES } from "../../../data/character-constants.ts";
import ClassCard from "../cards/ClassCard.tsx";
import type { Character, SrdClass } from "../../../types/index.ts";
...
interface Props {
  ...
  setPreviewClass: (id: string | null) => void;
  classCarouselRef: RefObject<HTMLDivElement | null>;
  classCarouselOverflows: boolean;
  classData: SrdClass | null;
}

export default function ClassStep({ char, update, next, prev, haptic, previewClass, setPreviewClass, classCarouselRef, classCarouselOverflows, classData }: Props) {
  const isMobile = useIsMobile();
  const [closing, setClosing] = useState<boolean>(false);

  const pCls = previewClass ? (classesData as SrdClass[]).find((c) => c.id === previewClass) : null;
  const pImg = previewClass ? (CLASS_IMAGES as Record<string, string>)[previewClass] : null;
    ...
    return (
      <div>
        ...
        <div style={{ position: "relative" }}>
          ...
          >
            {(classesData as SrdClass[]).map((cls) => {
