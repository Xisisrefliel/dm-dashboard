...
import { CAMPAIGN_COLORS } from "../data/sampleCampaign.ts";
import type { CampaignSummary } from "../types/index.ts";
...
function CampaignHome({ campaigns, onSelect, onCreate, onDelete, onRename, user, onLogout, onCharacterCreator }: CampaignHomeProps) {
  const [creating, setCreating] = useState<boolean>(false);
  const [newName, setNewName] = useState<string>("");
  const [newDesc, setNewDesc] = useState<string>("");
  const [newColor, setNewColor] = useState<string>(CAMPAIGN_COLORS[0] ?? "#9fd494");

  // Context menu
  const [ctxMenu, setCtxMenu] = useState<CtxMenuState | null>(null);

  // Rename modal
  const [renaming, setRenaming] = useState<CampaignSummary | null>(null);
  ...
  const handleCreate = () => {
    ...
    setNewName("");
    setNewDesc("");
    setNewColor(CAMPAIGN_COLORS[0] ?? "#9fd494");
    setCreating(false);
  };
  ...
  return (
    <div
      style={{
        "--dm-bg": "#141518",
        "--dm-surface": "#1d1f24",
        "--dm-surface-bright": "#272a31",
        "--dm-text": "#e7e8ec",
        "--dm-text-secondary": "#c4c7d0",
        "--dm-text-muted": "#8d919b",
        "--dm-primary": "#c7c9d1",
        "--dm-outline": "#8d919b",
        "--dm-outline-variant": "#3a3d46",
        "--dm-surface-brighter": "#31343c",
        "--dm-error": "#ffb4ab",
