...
export function SRDRulesDrawer({ Icon, Ripple, selectedSection, onSelectSection }) {
  const [expanded, setExpanded] = useState(rulesData[0]?.id || null);
      ...
      <div style={{ flex: 1, overflow: "auto", padding: "0 12px" }}>
        {filteredRules.map(rule => (
          <div key={rule.id} style={{ marginBottom: 2 }}>
            ...
            {expanded === rule.id && rule.sections.map((section, i) => (
              <Ripple
                key={section.id}
...
export function SRDRulesContent({ sectionId, Icon, Ripple, renderMarkdown, onPin, isPinned }) {
  ...
  const pinId = `srd-rule-${currentSection.id}`;
