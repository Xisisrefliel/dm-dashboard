...
import type { SrdRace, SrdAbilityBonus, SrdRaceTrait } from "../types/index.ts";
import { racesData, RACE_ICONS } from "./RaceList.tsx";

interface RacePinData {
  id: string;
  title: string;
  icon: string;
  category: string;
...
interface RaceDetailProps {
  ...
  onPin: (data: RacePinData) => void;
  isPinned: (id: string) => boolean;
}

export default function RaceDetail({ raceId, Icon, Ripple, onPin, isPinned }: RaceDetailProps): React.ReactElement {
  const race: SrdRace | undefined = racesData.find(r => r.id === raceId);
  if (!race) return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          {["STR", "DEX", "CON", "INT", "WIS", "CHA"].map(ability => {
            const bonus: SrdAbilityBonus | undefined = race.abilityBonuses.find(b => b.ability === ability);
            return (
              <div key={ability}>
        ...
        {race.traits.length > 0 && (
          <>
            ...
            {race.traits.map((trait: SrdRaceTrait, i: number) => (
        ...
        {race.subraces.length > 0 && (
          <>
            {(race.subraces as Array<{ name: string; desc: string; abilityBonuses?: SrdAbilityBonus[]; traits?: SrdRaceTrait[] }>).map((sub, si: number) => (
              <div key={si} style={{ marginTop: 20 }}>
                ...
                {sub.abilityBonuses && sub.abilityBonuses.length > 0 && (
                ...
                {sub.traits?.map((trait: SrdRaceTrait, ti: number) => (
