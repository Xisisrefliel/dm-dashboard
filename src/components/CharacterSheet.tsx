...
import { loadCharacters, saveCharacters } from "../utils/characterStorage.ts";
import { RACE_IMAGES, CLASS_IMAGES } from "../data/character-images.ts";
import { BACKGROUNDS, ALIGNMENTS, ABILITIES, ABILITY_NAMES, ABILITY_ICONS, SRD_ITEMS } from "../data/character-constants.ts";
import { styles, ms } from "./character-creator/styles.ts";
import TraitChip from "./character-creator/cards/TraitChip.tsx";
import type { Character, InventoryItem } from "../types/index.ts";

interface RaceDataType {
  id: string;
  name: string;
  speed: number;
...
interface CharacterSheetProps {
  ...
  setAssignedStats: React.Dispatch<React.SetStateAction<Record<string, number>>>;
  raceData: RaceDataType | null;
  classData: ClassDataType | null;
  computedEquipment: EquipmentItem[];
...
export default function CharacterSheet({ char, update, onBack, editId, assignedStats, setAssignedStats, raceData, classData, computedEquipment, getRacialBonus, isMobile, haptic }: CharacterSheetProps) {
  ...
  const [spellFilter, setSpellFilter] = useState<string | number>("all");
  ...
  const saveEdits = () => {
    ...
    const charToSave = { ...char, assignedStats: assignedStats as any, id: editId || char.id || Date.now().toString() } as Character;
    ...
    <div style={{ ...styles.root, height: "auto", minHeight: isMobile ? "100dvh" : "100vh", overflow: "visible" }}>
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: isMobile ? "16px 16px" : "24px 20px" }}>
        ...
        {(() => {
          ...
          const doAttack = (name: string, diceStr: string, type: string, isSpell: boolean) => {
                    ...
                    <div style={{ marginBottom: char.spells.length > 0 ? 12 : 0 }}>
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                        {char.cantrips.map((id) => {
                          ...
                          return sp ? (
                            <Ripple key={id} onClick={() => { setSelectedSpell(sp as SpellData); setOpenPanel("spellbook"); }} style={{
                ...
                <div style={{
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                    {raceData!.traits!.map((t) => (
                      <TraitChip key={t.name} trait={{ name: t.name, desc: (t as any).desc || (t as any).description || "" }} asButton />
                    ))}
                  </div>
                </div>
              )}
