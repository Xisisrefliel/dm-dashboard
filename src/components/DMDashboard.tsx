...
import {
  SRDRulesDrawer,
  SRDRulesContent,
} from "./SRDRulesViewer.tsx";
import SpellList, { spellsData } from "./SpellList.tsx";
import SpellCard from "./SpellCard.tsx";
import MonsterList, { monstersData } from "./MonsterList.tsx";
import MonsterStatBlock from "./MonsterStatBlock.tsx";
import ClassList, { classesData } from "./ClassList.tsx";
import ClassDetail from "./ClassDetail.tsx";
import RaceList, { racesData } from "./RaceList.tsx";
import RaceDetail from "./RaceDetail.tsx";
import rulesData from "../data/srd-rules.json";
import Icon from "./ui/Icon.tsx";
import Ripple from "./ui/Ripple.tsx";
import Chip from "./ui/Chip.tsx";
import ContextMenu from "./ui/ContextMenu.tsx";
import { renderMarkdown, highlightMatch } from "../utils/renderMarkdown.tsx";
import { SRD_CATEGORIES, ICON_OPTIONS } from "../data/sampleCampaign.ts";
import { derivePalette, hexToRGB } from "../utils/colorUtils.ts";
import DiceRoller from "./DiceRoller.tsx";
import InitTracker from "./InitTracker.tsx";
...
interface SearchResultItem extends Partial<Doc> {
  id: string;
  title: string;
  icon: string;
  category: string;
...
function DMDashboard({ campaign, onBack }: DMDashboardProps) {
  ...
  const [pasteIcon, setPasteIcon] = useState<string>("description");
  ...
  const [selectedRuleSection, setSelectedRuleSection] = useState<string | null>(null);
  ...
  // Categories (mutable)
  const [categories, setCategories] = useState<Category[]>(campaign.categories);
  const [renamingCat, setRenamingCat] = useState<string | null>(null);
  const [renameCatValue, setRenameCatValue] = useState<string>("");
  ...
  const togglePin = (item: Doc) =>
    setPinned((p) =>
      p.find((x) => x.id === item.id)
        ? p.filter((x) => x.id !== item.id)
        : [...p, item],
    );
  const isPinnedCheck = (id: string) => pinned.some((x) => x.id === id);

  const addToInitiative = (creature: { name: string; hp: number | null }) => {
  ...
  const toggleShare = (item: Doc & { shared?: boolean }) => {
    const newShared = !item.shared;
    setDocs((d) => d.map((x) => x.id === item.id ? { ...x, shared: newShared } : x));
    if (doc?.id === item.id) setDoc((d) => ({ ...d!, shared: newShared } as any));
  ...
  const startAddChild = (parentItem: Doc) => {
    setPasteMode(true);
  ...
  const handleContextMenu = (e: React.MouseEvent, item: Doc) => {
    e.preventDefault();
            ...
            >
              ...
              <div style={{ flex: 1, overflow: "auto", padding: "0 12px" }}>
                ...
                {!isSRD && (
                  <>
                    {(() => {
                      const renderItem = (item: Doc, depth = 0): React.ReactNode => {
              ...
              ctxMenu.catItem
                ...
                : isOwner
                  ? [
                      ...
                      { divider: true },
                      {
                        icon: (ctxMenu.item as any)?.shared ? "visibility_off" : "groups",
                        label: (ctxMenu.item as any)?.shared ? "Unshare with party" : "Share with party",
                        action: () => toggleShare(ctxMenu.item as any),
                      },
                      { divider: true },
                      {
                        icon: "delete",
