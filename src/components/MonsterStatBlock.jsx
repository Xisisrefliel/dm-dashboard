import { monstersData, crDisplay } from "./MonsterList.jsx";
...
function ActionSection({ title, actions, Icon }) {
...
export default function MonsterStatBlock({ monsterId, Icon, Ripple, onPin, isPinned, onAddToInitiative }) {
  ...
  const pinId = `srd-monster-${monster.id}`;
  const speedStr = Object.entries(monster.speed).map(([k, v]) => k === "walk" ? v : `${k} ${v}`).join(", ");
  const savesStr = monster.savingThrows
  ...
  const skillsStr = monster.skills
    ...
    : null;
  const sensesStr = monster.senses
    ...
    <div style={{ maxWidth: 700, margin: "0 auto", padding: "32px 48px", animation: "m3pop 0.25s ease" }}>
      ...
      <div style={{
        ...
        <div style={{ padding: "4px 0 8px" }}>
          ...
          {monster.vulnerabilities && <StatLine label="Vulnerabilities" value={monster.vulnerabilities.join(", ")} />}
          {monster.resistances && <StatLine label="Resistances" value={monster.resistances.join(", ")} />}
          {monster.immunities && <StatLine label="Damage Immunities" value={monster.immunities.join(", ")} />}
          {monster.conditionImmunities && <StatLine label="Condition Immunities" value={monster.conditionImmunities.join(", ")} />}
